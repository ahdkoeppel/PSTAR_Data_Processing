%% Code for Reading in and Converting FLIR Duo Pro or Vue Pro tiff images to temperatures with option to use adjustable observation/atmospheric variables
% Ari Koeppel - 2021
clear
close all;

%% Import Data
%Place all your images of interest in a single folder
FLIRDir = uigetdir('~','Directory of FLIR .tiff files');%gets directory
FLIRFiles = dir(fullfile(FLIRDir,'*.tiff')); %gets all files

%% Direct conversion from digital number to temperature (C) with no radiometric calibration
% Saves output images as _TempCUncalibrated.tiff suffix
for k = 1:length(FLIRFiles)
  raw_thermal = fullfile(FLIRDir, FLIRFiles(k).name); %Loads full filename
  newfilename = [raw_thermal(1:end-5),'_TempCUncalibrated',raw_thermal(end-4:end)]; %Makes a new filename
...
% Saves output images as _TempC.tiff suffix
...
AtmosphericTemperature = 0; %in C
RelativeHumidity= 0; %0 to 100
